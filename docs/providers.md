...
> **Spring AI 2.0.0-M7 note** — `spring-ai-azure-openai` was removed in M5. Azure OpenAI is now served by the unified `spring-ai-openai` module under the **Microsoft Foundry** label. Foundry mode is auto-detected when `spring.ai.openai.base-url` ends with `openai.azure.com` (or `cognitiveservices.azure.com`), or when a `deployment-name` is set. Config keys moved from `spring.ai.azure.openai.*` → `spring.ai.openai.*`. See `migration/SPRING_AI_M4_TO_M5_MIGRATION.md` for the full migration record.
